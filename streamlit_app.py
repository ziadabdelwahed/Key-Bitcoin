...
# Load BIP-39 wordlist directly from file or embedded
BIP39_WORDS = [
...
def brainwallet_phrase(text):
    h = hashlib.sha256(text.encode()).digest()
    return ' '.join(BIP39_WORDS[((h[i] << 8) | h[(i+1) % len(h)]) % 2048] for i in range(12))

COMMON = ["password","12345678","qwerty123","letmein","bitcoin","ethereum","satoshi","metamask"]
...
st.caption("All processing is done locally. No external APIs.")

st.markdown("---")

tab1, tab2, tab3 = st.tabs(["🎲 Generate", "🔍 Analyze", "🧠 Brainwallet"])

with tab1:
    c1, c2 = st.columns(2)
    with c1:
        count = st.slider("Batch size", 1, 50, 5)
    with c2:
        wc = st.radio("Words", [12, 24], horizontal=True)
    
    if st.button("🎲 Generate", type="primary", use_container_width=True):
        phrases = [random_phrase(wc) for _ in range(count)]
...
with tab2:
    phrase = st.text_area("Enter mnemonic:", height=80, placeholder="word1 word2 ... word12")
    ...
    if st.button("🔍 Analyze", type="primary", use_container_width=True):
        if phrase.strip():
            ...
            if len(words) in [12, 24] and all(w in BIP39_WORDS for w in words):
                ...
                c2.metric("Entropy", f"{len(words) * 11} bits")
                c3.metric("Key Length", f"{len(key.hex())} hex chars")
...
with tab3:
    text = st.text_input("Brainwallet phrase:", placeholder="e.g. correct horse battery staple")
    
    if st.button("🧠 Derive", type="primary", use_container_width=True):
        if text.strip():
            phrase = brainwallet_phrase(text.strip())
            seed = mnemonic_to_seed(phrase)
            key = seed_to_key(seed)
            st.success("Brainwallet derived")
            st.code(phrase)
            st.text(f"Key: {key.hex()}")

st.markdown("---")
st.caption("Phantom Key Hunter | Local BIP-39 Tool | No dependencies")
