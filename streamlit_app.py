...
def private_key_to_btc_address(pk_hex: str) -> str:
    try:
        import coincurve
        pk = coincurve.PrivateKey(bytes.fromhex(pk_hex))
        return pk.public_key.address()
    except:
        return "N/A"

def private_key_to_eth_address(pk_hex: str) -> str:
    try:
...
COMMON_BRAINWALLETS = [
    ...
    "hello world", "admin123", "metamask", "trustwallet", "crypto"
]

COMMON_FIRST = [
    "abandon", "apple", "banana", "cat", "dog", "sun", "moon",
    "love", "hope", "life", "gold", "blue", "red", "king", "fire"
]

def random_phrase(n: int = 12) -> str:
    return ' '.join(BIP39_WORDS[secrets.randbelow(2048)] for _ in range(n))

def brainwallet_phrase(seed_text: str) -> str:
    h = hashlib.sha256(seed_text.encode()).digest()
    return ' '.join(BIP39_WORDS[((h[i] << 8) | h[(i+1)%len(h)]) % 2048] for i in range(12))

def sequential_phrase() -> str:
    s = secrets.randbelow(2048 - 12)
    return ' '.join(BIP39_WORDS[s:s+12])

def repeated_phrase() -> str:
    w = BIP39_WORDS[secrets.randbelow(2048)]
    return ' '.join([w] * 12)

def prefix_phrase(pref: str) -> str:
    if pref not in BIP39_WORDS:
        return ""
    return pref + ' ' + ' '.join(BIP39_WORDS[secrets.randbelow(2048)] for _ in range(11))

def generate_batch(n: int) -> List[str]:
    phrases = []
    for _ in range(int(n * 0.40)): phrases.append(random_phrase(12))
    for _ in range(int(n * 0.15)): phrases.append(brainwallet_phrase(secrets.choice(COMMON_BRAINWALLETS)))
    for _ in range(int(n * 0.10)): phrases.append(sequential_phrase())
    for _ in range(int(n * 0.10)): phrases.append(prefix_phrase(secrets.choice(COMMON_FIRST)))
    for _ in range(int(n * 0.10)): phrases.append(repeated_phrase())
    for _ in range(int(n * 0.10)): phrases.append(random_phrase(24))
    for _ in range(int(n * 0.05)): phrases.append(random_phrase(24))
    return phrases
...
st.set_page_config(page_title="Phantom Key Hunter", page_icon="🔑", layout="wide")

st.title("🔑 Phantom Key Hunter")
st.subheader("BIP-39 Mnemonic Scanner — BTC | ETH | Multi-Chain")
st.caption("Research tool for cryptographic analysis of BIP-39 key space")

st.markdown("---")

tab1, tab2, tab3 = st.tabs(["🎲 Generate", "🔍 Scan", "📊 Results"])
...
with tab1:
    ...
    with col1:
        count = st.number_input("Batch size", min_value=1, max_value=10000, value=10)
    with col2:
        gen_type = st.selectbox("Type", [
            "Mixed (All Strategies)",
            "Random (12 words)",
            "Random (24 words)",
            "Brainwallet-derived",
            "Sequential",
    ...
    with col3:
        if st.button("Generate", type="primary", use_container_width=True):
            if "Mixed" in gen_type:
                phrases = generate_batch(count)
            elif "Random (12" in gen_type:
                phrases = [random_phrase(12) for _ in range(count)]
            elif "Random (24" in gen_type:
                phrases = [random_phrase(24) for _ in range(count)]
            elif "Brainwallet" in gen_type:
                phrases = [brainwallet_phrase(secrets.choice(COMMON_BRAINWALLETS)) for _ in range(count)]
            elif "Sequential" in gen_type:
                phrases = [sequential_phrase() for _ in range(count)]
            elif "Repeated" in gen_type:
                phrases = [repeated_phrase() for _ in range(count)]
            elif "Prefix" in gen_type:
                phrases = [prefix_phrase(secrets.choice(COMMON_FIRST)) for _ in range(count) if prefix_phrase(secrets.choice(COMMON_FIRST))]
            else:
                phrases = []
            
            st.session_state['phrases'] = phrases
            st.success(f"Generated {len(phrases)} phrases")

    if 'phrases' in st.session_state and st.session_state['phrases']:
        st.markdown("### Generated Phrases")
        for i, phrase in enumerate(st.session_state['phrases'][:20]):
            with st.expander(f"Phrase {i+1}: {phrase[:50]}..."):
                st.code(phrase)
                
                seed = mnemonic_to_seed(phrase)
                pk = seed_to_master_key(seed)
                pk_hex = pk.hex()
                
                c1, c2 = st.columns(2)
                with c1:
                    st.text(f"Private Key: {pk_hex[:32]}...")
                    st.text(f"BTC: {private_key_to_btc_address(pk_hex)}")
                with c2:
                    st.text(f"Seed: {seed.hex()[:32]}...")
                    st.text(f"ETH: {private_key_to_eth_address(pk_hex)}")

with tab2:
    st.subheader("Balance Scanner")
    
    phrase_input = st.text_area("Paste mnemonic phrase:", height=80,
                                 placeholder="word1 word2 word3 ... word12")
    
    if st.button("Scan Balance", type="primary", use_container_width=True):
        if phrase_input.strip():
            words = phrase_input.strip().split()
            if len(words) in [12, 15, 18, 21, 24] and all(w in BIP39_WORDS for w in words):
                ...
                with c2:
                    st.metric("Private Key", pk_hex[:16] + "...")
                with c3:
                    btc_addr = private_key_to_btc_address(pk_hex)
                    eth_addr = private_key_to_eth_address(pk_hex)
                
                st.markdown("### Derived Addresses")
                st.text(f"BTC: {btc_addr}")
                st.text(f"ETH: {eth_addr}")
                
                st.info("Balance checking requires API keys. Add in Settings tab.")
            else:
                st.error("Invalid BIP-39 mnemonic")

with tab3:
    st.subheader("Session Results")
    
    if 'found_wallets' not in st.session_state:
        st.session_state['found_wallets'] = []
    
    if not st.session_state['found_wallets']:
        st.info("No wallets scanned yet. Use the Generate tab to create phrases and Scan tab to check balances.")
    else:
        st.dataframe(st.session_state['found_wallets'])

st.markdown("---")
st.caption("Phantom Key Hunter — BIP-39 Research Tool | Use only on wallets you own")
